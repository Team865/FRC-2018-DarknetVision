...
char* draw_detections_cv_v3(IplImage* show_img, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output, char* data);
...
void *detect_in_thread(void *ptr)
{
	...
	char* b = "";
	b = draw_detections_cv_v3(det_img, dets, nboxes, demo_thresh, demo_names, demo_alphabet, demo_classes, demo_ext_output, b);
	asprintf(&hookedMemaddr, "%s%s", a, b);
	hookedMemaddrLength = strlen(hookedMemaddr);
	//printf("%s",hookedMemaddr);
	//printf("%d\n",hookedMemaddrLength);
...
void demo(char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, char **names, int classes,
    int frame_skip, char *prefix, char *out_filename, int http_stream_port, int dont_show, int ext_output, char* exeName)
{
